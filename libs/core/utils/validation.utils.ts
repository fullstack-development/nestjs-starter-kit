...
export function IsAddressDecorator(validationOptions?: ValidationOptions) {
    // eslint-disable-next-line @typescript-eslint/ban-types
    return function (object: object, propertyName: string) {
        registerDecorator({
            target: object.constructor,
...
export const IsAddress = (validationOptions?: ValidationOptions) =>
    applyDecorators(
        Transform(({ value }) => {
            if (typeof value === 'string') {
        ...
        IsAddressDecorator(validationOptions),
    );
...
export const isNotEmpty = <T>(arr: Array<T>): arr is Array<T> & { 0: T } => arr.length > 0;
